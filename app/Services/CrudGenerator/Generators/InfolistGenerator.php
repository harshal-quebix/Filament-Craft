...
class InfolistGenerator implements GeneratorInterface
{
    ...
    private function buildEntries(array $fields, array $relationships, string $modelName): array
    {
        ...
        foreach ($fields as $field) {
            $entry = $this->buildEntry($field, $modelName);
    ...
    private function buildEntry(array $field, string $modelName): ?string
    {
    ...
    private function buildRelationshipEntry(array $rel): ?string
    {
        ...
        if (! in_array($type, ['belongsTo', 'hasOne'])) return null;
    ...
    private function buildImports(array $fields): string
    {
        ...
        foreach ($fields as $field) {
            ...
            match ($ht) {
                'toggle' => $imports[] = 'use Filament\\Infolists\\Components\\IconEntry;',
                'file'   => $imports[] = 'use Filament\\Infolists\\Components\\ImageEntry;',
                'color'  => $imports[] = 'use Filament\\Infolists\\Components\\ColorEntry;',
                default  => null,
            };
        }
