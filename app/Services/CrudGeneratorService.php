...
class CrudGeneratorService
{
    ...
    {
        try {
            $fieldsArray = $generator->fields ?? [];
            if (empty($fieldsArray) || ! is_array($fieldsArray) || count($fieldsArray) === 0) {
                throw ValidationException::withMessages([
                    'fields' => __('At least one field or relationship must be configured before generation.'),
                ]);
            }
            ...
            $this->translationService->generateLocalizationKeys($modelName, $fields);
    ...
    public function cleanup(Generator $generator): bool
    {
        ...
        $this->fileManager->deleteIfExists(config('crud-generator.paths.models') . "/{$modelName}.php");
        $this->fileManager->deleteDirectoryIfExists(config('crud-generator.paths.resources') . "/{$pluralName}");
        $this->fileManager->deleteIfExists(config('crud-generator.paths.resources') . "/{$modelName}Resource.php");
        ...
        foreach (array_merge(
            File::glob(config('crud-generator.paths.migrations') . '/*_create_' . $tableName . '_table.php'),
            File::glob(config('crud-generator.paths.migrations') . '/*_add_columns_to_' . $tableName . '_table.php')
        ) as $file) {
            $this->fileManager->deleteIfExists($file);
        }

        $this->fileManager->deleteIfExists(database_path("seeders/{$modelName}Seeder.php"));

        $split = $this->fieldResolver->splitFields($generator->fields ?? []);
        $this->translationService->removeLocalizationKeys($modelName, $split['fields']);
    ...
    public function regenerateFormWithUpdatedConfig(Generator $generator): void
    {
        $modelName = $generator->model_name;
        $pluralName = $this->pluralName($modelName);
        $formPath = config('crud-generator.paths.resources') . "/{$pluralName}/Schemas/{$modelName}Form.php";
    ...
    {
        $path = config('crud-generator.paths.models') . "/{$modelName}.php";
        $content = (new ModelGenerator(new StubRenderer(), $this->fieldResolver))->generate([
