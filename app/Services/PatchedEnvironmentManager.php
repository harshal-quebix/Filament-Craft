<?php

namespace App\Services;

use Illuminate\Http\Request;
use Illuminate\Support\Str;
use RachidLaasri\LaravelInstaller\Helpers\EnvironmentManager;

class PatchedEnvironmentManager extends EnvironmentManager
{
    /**
     * Override saveFileWizard to write correct Laravel 12 env keys
     * instead of the old Laravel 5.x keys the package uses.
     */
    public function saveFileWizard(Request $request): string
    {
        $results = trans('installer_messages.environment.success');

        $envFileData =
            "APP_NAME='" . $request->app_name . "'\n" .
            "APP_ENV=" . $request->environment . "\n" .
            "APP_KEY=base64:" . base64_encode(Str::random(32)) . "\n" .
            "APP_DEBUG=" . $request->app_debug . "\n" .
            "APP_LOG_LEVEL=" . $request->app_log_level . "\n" .
            "APP_URL=" . $request->app_url . "\n" .
            "APP_LOCALE=en\n" .
            "APP_TIMEZONE=UTC\n\n" .
            "LOG_CHANNEL=stack\n" .
            "LOG_LEVEL=" . $request->app_log_level . "\n\n" .
            "DB_CONNECTION=" . $request->database_connection . "\n" .
            "DB_HOST=" . $request->database_hostname . "\n" .
            "DB_PORT=" . $request->database_port . "\n" .
            "DB_DATABASE=" . $request->database_name . "\n" .
            "DB_USERNAME=" . $request->database_username . "\n" .
            "DB_PASSWORD=" . $request->database_password . "\n\n" .
            "BROADCAST_CONNECTION=" . $request->broadcast_driver . "\n" .
            "CACHE_STORE=" . $request->cache_driver . "\n" .
            "SESSION_DRIVER=" . $request->session_driver . "\n" .
            "SESSION_LIFETIME=120\n" .
            "QUEUE_CONNECTION=" . $request->queue_driver . "\n\n" .
            "REDIS_HOST=" . $request->redis_hostname . "\n" .
            "REDIS_PASSWORD=" . $request->redis_password . "\n" .
            "REDIS_PORT=" . $request->redis_port . "\n\n" .
            "MAIL_MAILER=" . $request->mail_driver . "\n" .
            "MAIL_HOST=" . $request->mail_host . "\n" .
            "MAIL_PORT=" . $request->mail_port . "\n" .
            "MAIL_USERNAME=" . $request->mail_username . "\n" .
            "MAIL_PASSWORD=" . $request->mail_password . "\n" .
            "MAIL_ENCRYPTION=" . $request->mail_encryption . "\n" .
            "MAIL_FROM_ADDRESS=hello@example.com\n" .
            "MAIL_FROM_NAME=\"\${APP_NAME}\"\n\n" .
            "FILESYSTEM_DISK=local\n\n" .
            "PUSHER_APP_ID=" . $request->pusher_app_id . "\n" .
            "PUSHER_APP_KEY=" . $request->pusher_app_key . "\n" .
            "PUSHER_APP_SECRET=" . $request->pusher_app_secret . "\n\n" .
            "VITE_APP_NAME=\"\${APP_NAME}\"\n";

        try {
            file_put_contents($this->getEnvPath(), $envFileData);
        } catch (\Exception $e) {
            $results = trans('installer_messages.environment.errors');
        }

        return $results;
    }
}
